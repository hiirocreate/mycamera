...
const installTipClose = $('installTipClose');

// ---- 「ホーム画面に追加」の案内（ブラウザ表示のときだけ出す） ----
function isStandalone() {
  return window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
}
if (!isStandalone()) {
  try {
    if (!localStorage.getItem('mycamera_installtip_dismissed')) {
...
installTipClose.addEventListener('click', () => {
  ...
  try { localStorage.setItem('mycamera_installtip_dismissed', '1'); } catch (e) {}
});
...
function takePhoto() {
  ...
  canvas.toBlob((blob) => {
    if (blob) saveMedia(blob, `photo_${Date.now()}.jpg`, 'image/jpeg');
  }, 'image/jpeg', 0.92);
}
...
async function startRecording() {
  ...
  try {
    ...
    showMessage('このブラウザでは動画録画に対応していません。');
    return;
  }
  ...
  state.mediaRecorder.onstop = () => {
    ...
    saveMedia(blob, `video_${Date.now()}.${ext}`, type);
  };
...
// ---- 保存（iOS Safariでは共有シート経由で「写真に保存」できるようにする） ----
async function saveMedia(blob, filename, mimeType) {
  try {
    ...
    console.warn('共有に失敗、ダウンロードに切り替えます', err);
  }
